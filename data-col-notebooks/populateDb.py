...
# api url
api_url = "https://denniesbor.com/gis/bulk_update/update_markers/"
    ...
    for file in files:
        if file.endswith(".geojson"):
            ...
            with open(os.path.join(root, file), 'r') as f:
                ...
                markers = []
                for i, marker in enumerate(data['features'][0]['geometry']['coordinates']):
                    try:    
                        markers.append({
                            'label': data['features'][0]['properties']['markers'][i]['type'],
                            'latitude': marker[1],
                            'longitude': marker[0]
                        })
                    except KeyError:
                        markers.append({
                            'label': data['features'][0]['properties']['markers'][i]['label'],
                            'latitude': marker[1],
                            'longitude': marker[0]
                        })
